...
void AddBtn(GtkWidget *pWindow, gint iExemple)
{
    ...
    case EXEMPLE_3:
        ...
        pButtonBox = gtk_button_box_new(GTK_ORIENTATION_HORIZONTAL);
        pLabel = gtk_label_new("QUIT");
        pImage = gtk_image_new_from_icon_name("application-exit", GTK_ICON_SIZE_BUTTON);
        gtk_button_box_set_layout(GTK_BUTTON_BOX(pButtonBox), GTK_BUTTONBOX_START);

        gtk_container_add(GTK_CONTAINER(pQuitBtn), pButtonBox);
        gtk_container_add(GTK_CONTAINER(pButtonBox), pLabel);
        gtk_container_add(GTK_CONTAINER(pButtonBox), pImage);
        break;
    }
